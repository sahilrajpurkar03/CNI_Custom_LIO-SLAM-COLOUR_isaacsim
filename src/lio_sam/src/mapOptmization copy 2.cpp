        ...
        std::for_each(camera_type_stdmap_.begin(), camera_type_stdmap_.end(),
                      [this, msgIn](const std::pair<std::string, color_point_cloud::CameraTypePtr> &pair) {
                          ...
                          pair.second->set_cv_image(pair.second->get_image_msg());
                            ...
                            //         if (!cv_image.empty()) {
                            //             cv::Mat resized;
                            //             cv::resize(cv_image, resized, cv::Size(640, 480));  // Resize to uniform size
                            ...
                            //         // Fill in missing images with black
                            ...
                            // // === DEBUG 31: Show Projected LiDAR Points in Grid View ===
                            ...
                            //     cv::Mat proj_image = cv_image.clone();

                            //     color_point_cloud::PointCloudConst cloud_debug{msgIn->cloud_corner};  // You can also add cloud_surface if needed

                            //     for (size_t i = 0; i < cloud_debug.getPointCount(); ++i) {
                            //         color_point_cloud::Point point{cloud_debug.getCurrentPoint()};
                            //         Eigen::Vector4d point4d(point.x, point.y, point.z, 1.0);
                            //         Eigen::Vector3d proj = pair.second->get_lidar_to_camera_projection_matrix() * point4d;

                            //         if (proj[2] <= 0) {
                            //             cloud_debug.nextPoint();
                            //             continue;
                            //         }
                            ...
                            //             cv::circle(proj_image, cv::Point(x, y), 2, cv::Scalar(0, 255, 0), -1);
                            //         }

                            //         cloud_debug.nextPoint();
                            //     }

                            //     // Resize to 640x480 for grid display
                            //     cv::Mat resized_proj;
                            //     cv::resize(proj_image, resized_proj, cv::Size(640, 480));
                            //     projected_images.push_back(resized_proj);
                            // }

                            // // Grid visualization like Debug 30
                            // if (!projected_images.empty()) {
                            //     while (projected_images.size() < 4) {
                            //         projected_images.push_back(cv::Mat::zeros(480, 640, CV_8UC3));
                            //     }

                            //     cv::Mat top, bottom, full;
                            //     cv::hconcat(projected_images[0], projected_images[1], top);
                            //     cv::hconcat(projected_images[2], projected_images[3], bottom);
                            //     cv::vconcat(top, bottom, full);

                            //     cv::imshow("Projected LiDAR Points on Camera Feeds", full);
                            //     cv::waitKey(1);
                            // }
                            // // === DEBUG 31 END ===
                            ...
                            int inside_image_count = 0;  // <-- new counter
                            int colored_count = 0;
                            int skipped_count = 0;
                            ...
                            for (size_t i = 0; i < cloud_corner.getPointCount(); ++i) {
                                ...
                                if (!std::isfinite(point3d_transformed_camera[0]) ||
                                    !std::isfinite(point3d_transformed_camera[1]) ||
                                    !std::isfinite(point3d_transformed_camera[2])) {
                                    RCLCPP_WARN(this->get_logger(), 
                                        "Invalid projection for point index %lu in camera %s. Values: [%f, %f, %f]", 
                                        i, pair.first.c_str(),
                                        point3d_transformed_camera[0], 
                                        point3d_transformed_camera[1], 
                                ...
                                if (x >= 0 && x < pair.second->get_image_width() &&
                                    ...
                                    #pragma omp atomic
                                    colored_count++;  // ✅ Count colored points
                                } else {
                                    #pragma omp atomic
                                    skipped_count++;  // ❌ Count skipped points
                                }
                            ...
                            // Clone the current camera image for debugging
                            cv::Mat debug_image = camera->get_cv_image().clone();

                            int max_points_to_draw = 1000;  // Limit how many points you draw for performance
                            int drawn_points = 0;

                            for (size_t i = 0; i < cloud_corner.getPointCount() && drawn_points < max_points_to_draw; ++i) {
                                color_point_cloud::Point point{cloud_corner.getCurrentPoint()};

                                Eigen::Vector4d point4d(point.x, point.y, point.z, 1.0);
                                Eigen::Vector3d pt_cam = camera->get_lidar_to_camera_projection_matrix() * point4d;

                                if (pt_cam[2] <= 0) { // Point behind camera
                                    cloud_corner.nextPoint();
                                    continue;
                                }

                                int x = static_cast<int>(pt_cam[0] / pt_cam[2]);
                                int y = static_cast<int>(pt_cam[1] / pt_cam[2]);

                                if (x >= 0 && x < debug_image.cols && y >= 0 && y < debug_image.rows) {
                                    // Draw a small circle on the image at projected location
                                    cv::circle(debug_image, cv::Point(x, y), 2, cv::Scalar(0, 255, 0), -1);
                                    drawn_points++;
                                }
                                cloud_corner.nextPoint();
                            }

                            // Save the debug image with projected points overlay
                            std::string debug_image_path = "/tmp/projected_points_on_" + pair.first + ".png";
                            cv::imwrite(debug_image_path, debug_image);

                            // Optionally log where you saved it
                            RCLCPP_INFO(this->get_logger(), "Saved projected lidar points on image to: %s", debug_image_path.c_str());                            

                            RCLCPP_INFO(this->get_logger(),
                                        "Camera [%s]: Points inside image: %d, Colored points: %d, Skipped points: %d",
                                        pair.first.c_str(), inside_image_count, colored_count, skipped_count);
                      });
